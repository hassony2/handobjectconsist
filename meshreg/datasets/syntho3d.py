...
class SynthHO3D:
    def __init__(
        ...
        self.corners = ho3dfullutils.load_corners(os.path.join(self.ho3d_root, "corners"))
    ...
    def get_objcorners2d(self, idx):
        ...
        return self.project(corners3d, idx)
    ...
    def get_obj_corners3d(self, idx):
        obj_id = self.sample_ids[idx]
        corners = self.corners[obj_id]
        obj_transform = self.obj_transforms[idx]
        if self.apply_obj_transform:
            obj_transform = self.obj_transforms[idx]
            hom_corners = np.concatenate([corners, np.ones([corners.shape[0], 1])], axis=1)
            trans_corners = obj_transform.dot(hom_corners.T).T[:, :3]
            trans_corners = self.cam_extrs[idx][:3, :3].dot(trans_corners.transpose()).transpose()
        else:
    ...
    def get_obj_corners_can(self, idx):
        obj_id = self.sample_ids[idx]
        corners = self.corners[obj_id]
        corners = self.cam_extrs[idx][:3, :3].dot(corners.transpose()).transpose()
